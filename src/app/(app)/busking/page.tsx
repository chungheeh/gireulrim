...
import { Plus, Search, Play, FileText, Music2, Star, Guitar } from "lucide-react";
...
type Filter = "전체" | "솔로" | "듀엣 모집" | "단체곡" | "셋리스트 확정";
...
function YoutubeThumbnail({ url, title }: { url: string | null; title: string }) {
  ...
  return (
    <div className="h-[68px] w-[120px] shrink-0 overflow-hidden rounded-xl bg-gradient-to-br from-gray-200 to-gray-300 flex items-center justify-center">
...
export default function BuskingPage() {
  ...
  const filters: Filter[] = ["전체", "솔로", "듀엣 모집", "단체곡", "셋리스트 확정"];
  ...
  const fetchSongs = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("songs")
        .select("*, users(name)")
        .order("is_busking_selected", { ascending: false })
  ...
  const filtered = songs.filter((s) => {
    const matchTag =
      filter === "전체" ? true :
      filter === "셋리스트 확정" ? s.is_busking_selected :
      s.tag === filter;
    const matchSearch =
  ...
  const confirmedCount = songs.filter(s => s.is_busking_selected).length;
  ...
  return (
    <>
      ...
      <div className="px-4 py-4 space-y-3">
        {/* 버스킹 현황 배너 */}
        <div className="rounded-2xl bg-gradient-to-br from-green-600 to-green-700 p-4">
          <div className="flex items-center gap-2 mb-2">
            <Guitar size={18} className="text-white" />
            <span className="text-sm font-bold text-white">버스킹 선곡 현황</span>
          </div>
          <div className="flex gap-4">
            <div>
              <p className="text-[10px] text-green-300">전체 선곡</p>
              <p className="text-xl font-black text-white">{songs.length}<span className="text-sm font-normal ml-1">곡</span></p>
            </div>
            <div>
              <p className="text-[10px] text-green-300">셋리스트 확정</p>
              <p className="text-xl font-black text-yellow-300">{confirmedCount}<span className="text-sm font-normal ml-1">곡</span></p>
            </div>
          </div>
          <p className="text-[11px] text-green-200 mt-2">
            🎤 버스킹에 부를 곡을 추가하고 함께 연습해요
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center pt-16 text-center">
            ...
            <p className="text-base font-medium text-gray-700">선곡된 곡이 없어요</p>
            <p className="mt-1 text-sm text-gray-400">첫 번째로 곡을 추가해보세요 🎵</p>
          </div>
        ) : (
          <div className="space-y-2.5">
            {filtered.map((song) => (
              <div key={song.id} className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                {song.is_busking_selected && (
                  <div className="flex items-center gap-1.5 bg-yellow-500 px-3 py-1.5">
                    ...
                    <span className="text-[11px] font-bold text-yellow-900">셋리스트 확정</span>
                  </div>
                )}
                <div className="p-3.5">
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-bold text-gray-900 leading-tight line-clamp-1">{song.title}</p>
                  ...
                  <div className="flex items-center gap-1.5 mt-2.5 pt-2.5 border-t border-gray-100">
                    ...
                    <span className="text-xs font-medium text-gray-600">{song.users?.name ?? "알 수 없음"}</span>
                    <span className="ml-auto text-[10px] text-gray-400">{formatDate(song.created_at)}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <AddSongSheet open={showAdd} onClose={() => setShowAdd(false)} onAdded={fetchSongs} />
    </>
  );
}
